...
/**
 * The two questions a chat reply must answer before anything reads it: it is
 * not an empty answer, and its finish reason is one the engine can act on.
 *
 * A tool-call reply is neither empty nor stop-shaped — DeepSeek's `content` is
 * null there and its finish reason is `tool_calls` — and both are exactly what
 * a round that will run tools looks like. Refusing it here was the second of
 * the two lines that kept the whole agent path shut, after the request side
 * stopped refusing tools: the round never saw a call to run.
 */
internal fun chatReplyRefusal(text: String, finish: String, toolCallCount: Int): String? = when {
    text.isBlank() && toolCallCount == 0 -> "E_COMPLETION_EMPTY_RESPONSE"
    finish !in setOf("stop", "length") && !(finish == "tool_calls" && toolCallCount > 0) ->
        ...
        try {
            ...
            when(protocol) {
                "chat-completions" -> {
                    ...
                    // Tool calls are not refused: a reply that carries them is
                    // a successful round, and its call count is what says so.
                    // They are still not read here — turning untrusted model
                    // output into calls is `completion_response`'s rule.
                    toolCallCount = message.optJSONArray("tool_calls")?.length() ?: 0
            ...
            chatReplyRefusal(text, finish, toolCallCount)?.let { fail(it) }
