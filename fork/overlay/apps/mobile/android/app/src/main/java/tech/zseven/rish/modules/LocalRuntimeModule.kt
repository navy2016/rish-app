...
class LocalRuntimeModule(private val react: ReactApplicationContext) : ReactContextBaseJavaModule(react) {
    ...
    private fun reject(promise: Promise, error: Exception) {
        val failure = error as? RuntimeFailure
        val code = failure?.code ?: "E_COMPLETION_NATIVE"
        // An HTTP failure is only actionable with its status code; provider
        // bodies, URLs with credentials and secret values stay out of the
        // message.
        val message = if (failure != null && failure.httpStatus != null) "$code (HTTP ${failure.httpStatus})" else code
    ...
    /**
     * Harness-aware bootstrap. The JS runtime probe asks for a proof of the
     * active harness, so select its credential slot first: proof() derives
     * active_harness from the selected slot.
     */
